...
import { Shield, Users, Scale, ChevronDown, X, UserCog } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const ROLE_OPTIONS: RoleOption[] = [
  {
    role: null,
    label: "Mon compte reel",
    description: "Super Admin - Acces complet",
    icon: Shield,
    color: "text-[#9b5cf6]",
  },
  {
    ...
    description: "Creation de contenu, multi-cabinets",
    icon: Users,
    color: "text-[#3b82f6]",
  },
  {
...
export function RoleSwitcher() {
  ...
  const currentOption =
    ROLE_OPTIONS.find((option) => option.role === simulatedRole) || ROLE_OPTIONS[0];
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          {isSimulating && (
            <Badge
              variant="outline"
              className="ml-1 h-6 rounded-full border-[#f4c65a] bg-[#fff4d0] px-2.5 text-[11px] font-medium text-[#dd7c11]"
            >
              Simulation
            </Badge>
          )}
      ...
      <DropdownMenuContent
        align="end"
        className="w-[430px] rounded-[24px] border border-[#e5e7f2] p-0 shadow-[0_20px_50px_rgba(80,89,125,0.18)]"
      >
        ...
        {ROLE_OPTIONS.map((option) => {
          ...
          return (
            ...
            >
              <Icon className={cn("mt-0.5 h-5 w-5 flex-shrink-0", option.color)} />
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  {isActive && (
                    <Badge
                      variant="secondary"
                      className="h-6 rounded-full bg-[#eef1fb] px-2.5 text-[11px] font-medium text-[#7b859f]"
                    >
                      Actif
                    </Badge>
                  )}
                </div>
                <p className="mt-1 text-[14px] text-[#98a0bb]">{option.description}</p>
              </div>
            </DropdownMenuItem>
          );
        })}
